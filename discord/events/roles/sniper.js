...
module.exports = {
  ...
  onNight: async (interaction, client, userId, db) => {
    client.users.send(userId ,{content: 'خب اگه دوس داری می تونی امشب یه شلیک انجام بدی!', components:[await player_list((value) => {return value.userId === userId}, interaction, db)]}).then(async msg => {
      await wait(60000)
      await msg.delete()
    })
  },
